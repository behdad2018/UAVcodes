% unsteady 3D wing lifting line method by B. Davoudi 5/12/2016
...
xw=repmat(x(Nx,:)+dlx*cos(aoa)+0.26*uinf*dt,tmax-1,1);
yw=repmat(y(Nx,:),tmax-1,1);
zw=repmat(z(Nx,:)-dlx*sin(aoa),tmax-1,1);
% dlw1=repmat(uinf*dt,tmax-1,2*Ny);
wake=0; % u=[0 0 0];
...
for t=1:tmax
    ...
    G(:,t)=Am*RHS;    % vorticity on the wing
    
    Gw(1+2*(t-1)*Ny:2*t*Ny)= G(1+2*(Nx-1)*Ny:2*Nx*Ny,t);
    %*...
    %(dlx+dly)/(uinf*dt+dly);        % vorticity on the wake i.e., its strength the same as TE pannel
    ...
    if t==1
        ...
        for j=1:2*Nw*Ny
            ...
            index1w=ceil(j/(2*Ny));
            index2w=j-(2*Ny)*(index1w-1);
            Gw2(j)=(uinf*dt+dly)*Gw(j)/(dlw1(index1w,index2w)+dly);  % fixing the value wake vorticity
        end
    end
    if t>1
        uw=wakevelocity(Nx,Ny,Nw,dlw1,dlx,dly,aoa,Lam,dih,x,y,z,xw,yw,zw,G(:,t),Gw2);
        uw1=reshape(uw(1,:,:),Nw,2*Ny)*0;
        vw1=reshape(uw(2,:,:),Nw,2*Ny)*0;
        ww1=reshape(uw(3,:,:),Nw,2*Ny)*0;
        %         dlw1(1:t-1,:)=(uw1+uinf)*dt;
        %         xw(1:t-1,:)=xw(1:t-1,:)+(uw1+uinf)*dt;
        %       % xw(1:t-1,:)=bsxfun(@minus,xw(1:t-1,:)+(uw1+uinf)*dt,0.3*uinf*dt);
        %         yw(1:t-1,:)=yw(1:t-1,:)+vw1*dt;
        %         zw(1:t-1,:)=zw(1:t-1,:)+ww1*dt;
    end
    if t>2
        dlw1(1:t-2,:)=(uw1(1:t-2,:)+uinf)*dt;
        xw(1:t-2,:)=xw(1:t-2,:)+(uw1(1:t-2,:)+uinf)*dt;
        yw(1:t-2,:)=yw(1:t-2,:)+vw1(1:t-2,:)*dt;
        zw(1:t-2,:)=zw(1:t-2,:)+ww1(1:t-2,:)*dt;
    end
    
    if t==1
        a=zeros(3,2*Nx*Ny);
        uw11(1,:)=zeros(1,2*Ny);
        dlw1(t,:)=(uw11+uinf)*dt;    % the most recent pannel has the length of u*delt
    else
        dlw1(t,:)=(uw11+uinf)*dt ;
    end
